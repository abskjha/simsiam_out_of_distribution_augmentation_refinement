...
GausBlur = GaussianBlur(kernel_size=224//20*2+1, sigma=(0.1, 50))
...
def cutout(img_full,cutout_size=0.5):
    h = cutout_size
    w = cutout_size
    ...
    x1 = img_full[x_:x_+h_,y_:y_+w_].copy()
    x2 = img_full
    x2[x_:x_+h_,y_:y_+w_] = np.zeros_like(x1)*255
    x1 = Image.fromarray(x1)
    x2 = Image.fromarray(x2)
    return (x1,x2)

def cutout_blur(img_full,cutout_size=0.5):
    h = cutout_size
    w = cutout_size
    ...
    x1 = img_full[x_:min(x_+h_,hi),y_:min(y_+w_,wi)].copy()
    x2 = img_full
    x2[x_:x_+h_,y_:y_+w_] = np.array(GausBlur(Image.fromarray(x1.astype('uint8'))))
#     x2 = inpaint.inpaint_biharmonic(x2, mask,multichannel=True)
    x1 = Image.fromarray(x1)
    x2 = Image.fromarray(x2)
    return (x1,x2)

class SimSiamTransform():
    def __init__(self, image_size, cutout_size=0.5, mean_std=imagenet_mean_std):
    ...
    def __call__(self, x):
        x1,x2 = cutout_blur(x,self.cutout_size)
