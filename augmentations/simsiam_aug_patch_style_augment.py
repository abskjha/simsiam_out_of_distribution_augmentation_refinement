...
class SimSiamTransform():
    def __init__(self, image_size, mean_std=imagenet_mean_std):
        ...
        p_blur = 0.3 if image_size > 32 else 0 
        # the paper didn't specify this, feel free to change this value
        # I use the setting from simclr which is 50% chance applying the gaussian blur
        # the 32 is prepared for cifar training where they disabled gaussian blur
        self.patch_size = 0.37
        ...
        self.transform_patch = T.Compose([
            T.RandomResizedCrop(image_size, scale=(self.patch_size,self.patch_size)),
            T.RandomHorizontalFlip(),
            T.RandomApply([T.ColorJitter(0.4,0.4,0.4,0.1)], p=0.8),
            T.RandomGrayscale(p=0.2),
            T.RandomApply([T.GaussianBlur(kernel_size=image_size//20*2+1, sigma=(0.1, 2.0))], p=p_blur),
        ...
        self.transform_whole = T.Compose([
            T.RandomResizedCrop(image_size, scale=(1.0, 1.0)),
            T.RandomHorizontalFlip(),
            T.RandomApply([T.ColorJitter(0.4,0.4,0.4,0.1)], p=0.8),
            T.RandomGrayscale(p=0.2),
        ...
        self.transform_just_crop = T.Compose([
            T.RandomResizedCrop(image_size, scale=(0.8, 0.8)),
        ])
    def __call__(self, x):
#         x = self.transform_just_crop(x)



        x = x.resize((256,256))
        im_torch = toTensor(x).unsqueeze(0)
        im_torch = im_torch.cuda()
        im_restyled = augmentor(im_torch)
        x = toPIL(im_restyled.squeeze().cpu())
        
        
        
        x1 = self.transform_whole(x.resize((32,32)))
        x2 = self.transform_patch(x.resize((32,32)))
